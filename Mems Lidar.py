...
class MEMS_Sensor(object):
    def __init__(self, parent_actor, carla_transform, image_width=800, v_fov=20, h_fov_total=90, h_fov_pc=70,
                 n_scan_lines=100,
        ...
        self.pixel_coords_scan_pattern = MEMS_Sensor.scan_pattern(n_scan_lines=n_scan_lines,
                                                                  ...
                                                                  h_fov_total=h_fov_total, h_fov_pc=h_fov_pc,
                                                                  v_fov=v_fov)
    ...
    def rot_transl_pc(self):
        rot_mat = R.from_euler("xyz",
                               [self.parent.get_transform().rotation.roll - self.sensor.get_transform().rotation.roll,
                                # roll
                                self.parent.get_transform().rotation.pitch - self.sensor.get_transform().rotation.pitch,
                                # pitch
                                - self.parent.get_transform().rotation.yaw + self.sensor.get_transform().rotation.yaw],
                               # yaw
                               degrees=True).as_matrix()
        self.lidar_pc = np.dot(rot_mat, self.lidar_pc)
        self.lidar_pc = (self.lidar_pc.T + [self.carla_transform.location.x, self.carla_transform.location.y,
                                            self.carla_transform.location.z]).T

    def noise(self):
        # Randomly dropping Points
        self.lidar_pc = self.lidar_pc[:,
                        np.random.choice(self.lidar_pc.shape[1], np.int32(self.lidar_pc.shape[1] * 0.95),
                                         replace=False)]
    ...
    @staticmethod
    def scan_pattern(n_scan_lines, n_points_pattern, width, height, h_fov_total=80, h_fov_pc=70, v_fov=30,
                     ramp_function=r_0, f=150):
        """
        See: https://docs.blickfeld.com/cube/v1.0.1/scan_pattern.html
        ...
        x = (width / (h_fov_total) * (h_mirror_list + h_fov_total / 2) - 1)[
            (h_mirror_list > - h_fov_pc / 2) & (h_mirror_list < h_fov_pc / 2)]
...
if __name__ == "__main__":
    ...
    try:
        car = world.spawn_actor(vehicle_bp,
                                carla.Transform(carla.Location(-82., 13.5, 1.3083857), carla.Rotation(0, 0, 0)))
        sleep(1)
    ...
    rot = rt_matrix(0, 0, -2, 0, 0,
                    0)  # Note that the MEMS LiDAR Data origin is at the origin of the car. We have to translate the data to the origin of the camera.
